...
<template>
  <div class="mb-4 d-md-none" v-for="(platf, platfIndex) in data.platform">
    ...
    <div v-dragscroll class="d-flex overflow-x-auto">
      <div class="card m-3" style="min-width: 200px" v-for="(time, timeIndex) in data.time">
        <div class="card-body">
          ...
          <template v-for="(type, typeIndex) in data.type">
            <div class="d-flex justify-content-between">
              ...
              <p>
                <b>{{ data.data[timeIndex + data.time.length * typeIndex * data.platform.length + platfIndex * data.time.length].total }}</b>
              </p>
            </div>
          </template>
        </div>
